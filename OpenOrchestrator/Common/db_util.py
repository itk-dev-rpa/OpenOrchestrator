...
def get_email_triggers() -> list[list[str]]:
    ...
    The format of each trigger is as follows:
    [process_name, email_folder, last_run, process_path, process_args, status_text, is_git_repo, blocking, id]
...
@catch_db_error
def begin_single_trigger(UUID: str) -> bool:
...
@catch_db_error
def set_single_trigger_status(UUID: str, status: int) -> None:
...
@catch_db_error
def begin_scheduled_trigger(UUID: str, next_run: datetime) -> None:
...
@catch_db_error
def set_scheduled_trigger_status(UUID: str, status: int) -> None:
